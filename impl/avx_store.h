...
void _mm256_store_ps(float *mem, __m256 a) {
    _mm_store_ps(mem, a.lo);
    _mm_store_ps(mem+4, a.hi);
}

void _mm256_store_si256(__m256i *mem, __m256i a) {
...
void _mm256_storeu2_m128(float *hi_addr, float *lo_addr, __m256 a) {
    _mm_storeu_ps(lo_addr, a.lo);
    _mm_storeu_ps(hi_addr, a.hi);
}

void _mm256_storeu2_m128d(double *hi_addr, double *lo_addr, __m256d a) {
...
void _mm256_maskstore_pd(double *mem_addr, __m256i __mask, __m256d __a) {
    uint64_t mask[4];
    double a[4];
    memcpy(mask, &__mask, sizeof(__mask));
    memcpy(a, &__a, sizeof(__a));
